  ...
  <div>
    <div class="bg-[#294231] py-10 px-6">
      <div class="max-w-5xl mx-auto">
        <p class="text-[#C4993C] text-xs font-bold tracking-widest uppercase mb-2">Support Our Ministry</p>
    ...
    <div class="max-w-5xl mx-auto py-12 px-6 space-y-12">
      ...
      <!-- Giving Form -->
      <div v-if="selectedOption" class="bg-white rounded shadow p-8">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-2xl font-bold font-serif text-[#2A1A0E]">
            {{ selectedOption === 'onetime' ? 'One-Time Gift' : selectedOption === 'monthly' ? 'Monthly Support' : 'Make a Pledge' }}
          </h3>
          <button @click="selectedOption = null" class="text-[#8C7050] hover:text-[#5C4230]">✕</button>
        </div>

        <form @submit.prevent="handleGift" class="space-y-4">
          <!-- Amount -->
          <div>
            <label class="block text-sm font-bold text-[#5C4230] mb-3">Amount *</label>
            <div class="flex gap-2 mb-3">
              <button v-for="amt in [25, 50, 100, 250]" :key="amt" type="button" @click="giftForm.amount = amt" 
                ...
                :class="giftForm.amount === amt ? 'bg-[#1A5C30] text-white border-[#1A5C30]' : 'border-[#C8B89A] text-[#2A1A0E] hover:bg-[#f5f0e8]'">
                ${{ amt }}
              </button>
            </div>
            <input v-model.number="giftForm.amount" type="number" placeholder="Other amount" required min="1" 
              class="w-full px-4 py-2 border border-[#C8B89A] rounded text-sm" />
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="giftForm.firstName" type="text" required class="w-full px-4 py-2 border border-[#C8B89A] rounded text-sm" />
            </div>
            <div>
              <label class="block text-sm font-bold text-[#5C4230] mb-2">Last Name *</label>
              <input v-model="giftForm.lastName" type="text" required class="w-full px-4 py-2 border border-[#C8B89A] rounded text-sm" />
            </div>
            <div class="col-span-2">
              <label class="block text-sm font-bold text-[#5C4230] mb-2">Email *</label>
              <input v-model="giftForm.email" type="email" required class="w-full px-4 py-2 border border-[#C8B89A] rounded text-sm" />
            </div>
            <div class="col-span-2">
              <label class="block text-sm font-bold text-[#5C4230] mb-2">Phone</label>
              <input v-model="giftForm.phone" type="tel" class="w-full px-4 py-2 border border-[#C8B89A] rounded text-sm" />
            </div>
          </div>

          <!-- Fund Selection -->
          <div>
            <label class="block text-sm font-bold text-[#5C4230] mb-2">Designate Gift To *</label>
            <select v-model="giftForm.fund" required class="w-full px-4 py-2 border border-[#C8B89A] rounded text-sm">
          ...
          <div>
            <label class="block text-sm font-bold text-[#5C4230] mb-2">Special Instructions or Notes</label>
            <textarea v-model="giftForm.notes" rows="3" class="w-full px-4 py-2 border border-[#C8B89A] rounded text-sm"></textarea>
          </div>

          <!-- Recurring (if monthly) -->
          <div v-if="selectedOption === 'monthly'" class="bg-[#f5f0e8] p-4 rounded">
            <label class="flex items-center gap-3">
              <input v-model="giftForm.autoRenew" type="checkbox" class="w-4 h-4" />
              <span class="text-sm text-[#2A1A0E] font-bold">Automatically renew each month</span>
            </label>
          </div>
      ...
      <div v-if="!selectedOption" class="space-y-6">
        <h3 class="text-2xl font-bold font-serif text-[#2A1A0E]">Giving FAQ</h3>
        
        <div class="space-y-4">
          <div class="border-l-4 border-[#C4993C] pl-4 py-2">
            <h4 class="font-bold text-[#2A1A0E] mb-2">Is my gift tax-deductible?</h4>
            <p class="text-[#5C4230] text-sm">Yes, our church is a 501(c)(3) nonprofit. All gifts are tax-deductible. You'll receive a receipt for your records.</p>
          </div>

          <div class="border-l-4 border-[#C4993C] pl-4 py-2">
            <h4 class="font-bold text-[#2A1A0E] mb-2">What payment methods do you accept?</h4>
            <p class="text-[#5C4230] text-sm">We accept credit cards, debit cards, and bank transfers. All transactions are secure and encrypted.</p>
          </div>

          <div class="border-l-4 border-[#C4993C] pl-4 py-2">
            <h4 class="font-bold text-[#2A1A0E] mb-2">Can I give anonymously?</h4>
            <p class="text-[#5C4230] text-sm">Yes. While we'd love to thank you personally, you can opt to remain anonymous during the giving process.</p>
          </div>

          <div class="border-l-4 border-[#C4993C] pl-4 py-2">
            <h4 class="font-bold text-[#2A1A0E] mb-2">How is my gift used?</h4>
            <p class="text-[#5C4230] text-sm">Gifts to the General Fund support ministry operations, staff, and outreach. Designated gifts go directly to the specified ministry area.</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-[#f5f0e8] rounded p-8">
        <h3 class="text-2xl font-bold font-serif text-[#2A1A0E] mb-4">Other Ways to Give</h3>
        <ul class="space-y-2 text-[#5C4230] text-sm">
